...
// shadcn primitives used by the modernized personal settings modal
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import { Plus, Trash2, Settings2, Repeat, Eye, Split, Minus } from 'lucide-react';
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {/* Modernized Personal settings modal. shadcn Dialog + Card sections
          ...
          (handleAutoDistributionEnabledChange, addDistributionRule,
          updateDistributionRule, removeDistributionRule,
          handleHideZeroBalanceBucketsChange, handlePersonalSplitEnabledChange,
          handlePersonalSplitDefaultDaysChange,
          handleEnableNegativeOffsetBucketChange,
          handleNegativeOffsetBucketChange, setShowPersonalSplitConfig) are
          called unchanged — only the layout changed. */}
      <Dialog open={showSettings} onOpenChange={(open) => setShowSettings(open)}>
        <DialogContent className="max-h-[85vh] w-[680px] max-w-[95vw] overflow-y-auto p-0">
          <DialogHeader className="border-b border-border px-6 pb-4 pt-6">
            <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col gap-1">
                <DialogTitle className="text-lg">Personal Settings</DialogTitle>
                <DialogDescription>
                  Configure automatic distribution, display options, and offsetting for your personal buckets.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
          ...
          <div className="space-y-4 px-6 py-5">
            {/* Section 1 — Auto Monthly Distribution */}
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <CardTitle className="text-sm">Auto monthly distribution</CardTitle>
                    <CardDescription className="text-xs">
                      Automatically transfer funds between buckets on the first login of each month.
                    </CardDescription>
                  </div>
                </div>
                <Switch
                  checked={autoDistributionEnabled}
                  onCheckedChange={(checked) => handleAutoDistributionEnabledChange(checked)}
                  aria-label="Enable auto monthly distribution"
                />
              </CardHeader>
              {autoDistributionEnabled && (
                <CardContent className="space-y-3 border-t border-border pt-4">
                  ...
                  {autoDistributionRules.length === 0 ? (
                    <div className="rounded-md border border-dashed border-border bg-muted/40 p-4 text-center text-sm text-muted-foreground">
                      No distribution rules yet. Click <span className="font-medium text-foreground">Add rule</span> to create one.
                    </div>
                  ) : (
                        ...
                        >
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <Select
                                value={rule.sourceBucket || undefined}
                                onValueChange={(value) =>
                                  updateDistributionRule(rule.id, 'sourceBucket', value)
                                }
                              >
                            ...
                            <div className="space-y-1">
                              ...
                              <Select
                                value={rule.destBucket || undefined}
                                onValueChange={(value) =>
                                  updateDistributionRule(rule.id, 'destBucket', value)
                                }
                              >
                                <SelectTrigger className="h-8 text-sm">
                          ...
                          {rule.amount && rule.sourceBucket && rule.destBucket && (
                            <div className="rounded-sm border border-primary/30 bg-primary/5 px-2 py-1 text-center text-xs font-medium text-primary">
                              ${rule.amount} from <span className="font-semibold">{rule.sourceBucket}</span> → <span className="font-semibold">{rule.destBucket}</span>
                            </div>
                          )}
                        </div>
                      ))}
            ...
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="space-y-1">
                    <CardTitle className="text-sm">Hide buckets with zero balance</CardTitle>
                    <CardDescription className="text-xs">
                      Buckets with a balance below one cent are hidden from the bucket grid.
                    </CardDescription>
                  </div>
                </div>
                <Switch
                  checked={hideZeroBalanceBuckets}
                  onCheckedChange={(checked) => handleHideZeroBalanceBucketsChange(checked)}
                  aria-label="Hide zero-balance buckets"
                />
              </CardHeader>
            </Card>

            {/* Section 3 — Personal split configuration */}
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary">
                  ...
                  <div className="space-y-1">
                    <CardTitle className="text-sm">Personal split configuration</CardTitle>
                    <CardDescription className="text-xs">
                ...
                <Switch
                  ...
                  onCheckedChange={(checked) => handlePersonalSplitEnabledChange(checked)}
                  aria-label="Enable personal split feature"
                />
              </CardHeader>
              {personalSplitEnabled && (
                <CardContent className="border-t border-border pt-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex items-center gap-2">
                      <Label htmlFor="personal-split-days" className="text-sm text-foreground">
                        Default lookback
                      </Label>
                      <Input
                        id="personal-split-days"
                        type="number"
                        min="1"
                        max="365"
                        value={personalSplitDefaultDays}
                        onChange={(e) =>
                          handlePersonalSplitDefaultDaysChange(parseInt(e.target.value, 10) || 7)
                        }
            ...
            {/* Section 4 — Negative bucket offsetting */}
            <Card>
              <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0 pb-3">
                <div className="flex items-start gap-3">
                  <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-primary/10 text-primary">
                  ...
                  <div className="space-y-1">
                    <CardTitle className="text-sm">Negative bucket offsetting</CardTitle>
                    <CardDescription className="text-xs">
                      Negative bucket amounts are excluded from the Categories Sum and deducted from the selected offset bucket.
                    </CardDescription>
                  </div>
                </div>
                <Switch
                  checked={enableNegativeOffsetBucket}
                  onCheckedChange={(checked) => handleEnableNegativeOffsetBucketChange(checked)}
                  aria-label="Enable negative bucket offsetting"
                />
              </CardHeader>
              {enableNegativeOffsetBucket && (
                <CardContent className="space-y-2 border-t border-border pt-4">
                  <Label htmlFor="negative-offset-bucket" className="text-sm font-medium text-foreground">
                    Offset bucket
                  </Label>
                  <Select
                    value={selectedNegativeOffsetBucket || undefined}
                    onValueChange={(value) => handleNegativeOffsetBucketChange(value)}
                  >
                    <SelectTrigger id="negative-offset-bucket" className="h-9 text-sm">
                  ...
                  {selectedNegativeOffsetBucket && (
                    <div className="rounded-sm border border-success/30 bg-success/10 px-2 py-1 text-xs font-medium text-success">
                      ✓ Negative amounts will be deducted from{' '}
                      <span className="font-semibold">{selectedNegativeOffsetBucket}</span>
                    </div>
                  )}
                </CardContent>
              )}
      ...
      {/* Personal Split Configuration Dialog.
          Wrapped in shadcn Dialog so it stacks correctly on top of the
          Personal Settings Dialog. Radix Dialog handles stacked modal
          z-ordering and pointer-events internally — the previous hand-
          written div was rendered outside the Dialog portal and got
          `pointer-events: none` from the open parent Dialog, which
          blocked every click on the close/Done buttons until the parent
          was closed first.

          onOpenChange routes close events (overlay click, ESC, X icon)
          through the existing handleClosePersonalSplitConfig handler so
          any cleanup logic still runs. */}
      <Dialog
        open={showPersonalSplitConfig}
        onOpenChange={(open) => {
          if (!open) handleClosePersonalSplitConfig();
        }}
      >
        <DialogContent className="max-h-[85vh] w-[720px] max-w-[95vw] overflow-y-auto p-0">
          <DialogHeader className="border-b border-border px-6 pb-4 pt-6">
            <div className="flex items-center gap-3">
              <div className="flex h-9 w-9 items-center justify-center rounded-md bg-primary/10 text-primary">
                <Split className="h-4 w-4" />
              ...
              <div className="flex flex-col gap-1">
                <DialogTitle className="text-lg">Personal Split Configuration</DialogTitle>
                <DialogDescription>
                  Map shared budget categories to your personal savings buckets.
                </DialogDescription>
              </div>
            </div>
          </DialogHeader>
